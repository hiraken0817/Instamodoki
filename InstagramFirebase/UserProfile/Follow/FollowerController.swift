...
class FollowerController:UICollectionViewController,UICollectionViewDelegateFlowLayout,FollowFollowerCellDelegate{
    let cellId = "cellId"
    
    static let updateFeedNotificationName = NSNotification.Name(rawValue: "UpdateFollower")
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "フォロワー"
    ...
    fileprivate func fetchUsers(){
        ...
        //ユーザー（自分以外もあり）がフォローしているユーザーを取得する
        Firestore.firestore().collection("follower").document(userId).collection("followerUser").getDocuments{ [weak self] (snapshot, err) in//
            ...
            for document in snapshot!.documents {
                ...
                Firestore.firestore().collection("users").document(uid).getDocument(source: .cache) { (document, error) in
...
//        NotificationCenter.default.post(name: FollowerController.updateFeedNotificationName, object: nil)
